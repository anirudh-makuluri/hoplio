...
export default function ChatBubble({
  ...
  if (message.isDate) {
    return (
      <div className="flex flex-row z-10 justify-center sticky top-0">
        <Badge className="bg-foreground hover:bg-foreground w-28 flex justify-center">
          {message.time}
        </Badge>
      </div>
    );
  ...
  function handleEmojiClick(data: {
    ...
    socket.emit(
      "chat_reaction_client_to_server",
      {
        reactionId: data.id,
        id: message.id,
  ...
  return (
    <div
      className={
        (isSelf ? "justify-end" : "justify-start") + " flex my-3 relative"
      }
    >
      <ContextMenu>
          ...
          <div className="flex flex-col gap-1">
            {!message.isConsecutiveMessage && (isGroup || isAIMessage) && (
              ...
              >
                <Avatar
                  className={
                    (isAIMessage ? "ring-2 ring-blue-500 " : "") + "h-10 w-10"
                  }
                >
                  <AvatarImage
            ...
            >
              ...
              <div className="flex flex-row gap-1 items-center">
                ...
                {message.isMsgEdited && (
                  <p className="opacity-65 text-[10px] italic">(Edited)</p>
                )}
      ...
      <div
        className={
          ...
          " absolute -bottom-5 flex flex-row gap-1 max-w-full overflow-x-auto"
        }
      >
...
function TextMessage({ message }: { message: ChatMessage | ChatDate }) {
  ...
  if (message.isEncrypted && !message.chatInfo) {
    return (
      <p className="italic opacity-70 text-yellow-600 dark:text-yellow-400">
...
function FileMessage({ message }: { message: ChatMessage | ChatDate }) {
  return (
    <a
      href={message.chatInfo || "#"}
      target="_blank"
      rel="noreferrer"
      className="flex min-w-[220px] items-center gap-3 rounded-md border border-border/60 bg-background/30 p-3 transition-colors hover:bg-background/50"
    >
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs opacity-70">Open attachment</p>
      </div>
    </a>
  );
}
